...
def amin(da,cf=1):
    #annual max
    xa = da.groupby('time.year').min().compute()
    return xa
...
def cal_tape(mod, obs): # total absolute percent error
    ape = [np.abs(mi - oi) / oi for mi, oi in zip(mod, obs)]
    return np.nansum(ape,axis=0)

def cal_tae(mod, obs): # total absolute error
    ae = [np.abs(mi - oi) for mi, oi in zip(mod, obs)]
    return np.nansum(ae,axis=0)

def cal_amp(mod, obs): # annual amplitude maximum - minimum 
    mamp = np.max(mod) - np.min(mod)
    oamp = np.max(obs) - np.min(obs)
    diff = np.abs(mamp - oamp)
...
def coef_var(data):
    # d = detrend(data) # when we use a longer record - need to detrend
    mn = data.mean(dim='year')
    st = data.std(dim='year')
    cv = st/mn
    return cv

def cal_e2sd(mod,obs):
    error2 = (mod-obs.mean(dim='year'))**2
    stdev = obs.std(dim='year')
    return np.sum(error2/stdev)
